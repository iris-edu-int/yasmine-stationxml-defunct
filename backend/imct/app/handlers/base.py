...
from imct.app.utils.date import datetime_to_utc, strptime
from imct.app.utils.facade import HandlerMixin
from imct.app.utils.ujson import json_dump, json_load
...
class ExtJsHandler(AsyncThreadMixin, BaseHandler):
            ...
            for filter_criteria in self.request_params['filter']:
                ...
                if 'anyMatch' in filter_criteria:
                    ...
                    if operator == 'like':
                        ...
                        query = query.filter(or_(field != value, field == None))  # @IgnorePep8
                    elif operator == 'lt':
                        query = query.filter(field < value)
                    elif operator == 'gt':
                        query = query.filter(field > value)
                        ...
                        query = query.filter(field == None)  # @IgnorePep8
                    elif operator == 'in':
    ...
    def get_list(self, *args, **kwargs):
        ...
        if self.send_total_count:
            ...
            count_query_result = count_query.count()
