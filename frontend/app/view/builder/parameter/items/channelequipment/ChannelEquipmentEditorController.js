Ext.define('imct.view.xml.builder.parameter.items.channelequipment.ChannelEquipmentEditorController', {
    ...
    initData: function () {
        ...
        this.getViewModel().set('installationDate', Ext.Date.parse(value.installation_date, IMCT.Globals.DateReadFormat, true));
        this.getViewModel().set('removalDate', Ext.Date.parse(value.removal_date, IMCT.Globals.DateReadFormat, true));
        ...
        if (value.calibration_dates) {
            ...
            value.calibration_dates.forEach(function (calibrationDate) {
                var date = new imct.view.xml.builder.parameter.items.channelequipment.CalibrationDate();
                date.set('value', Ext.Date.parse(calibrationDate, IMCT.Globals.DateReadFormat, true));
                date.modified = {};
                store.add(date);
            })
        }
    ...
    fillRecord: function () {
        ...
        store.getData().items.forEach(function (calibrationDate) {
            calibrationDates.push(Ext.Date.format(calibrationDate.getData().value, IMCT.Globals.DateReadFormat));
        })
        ...
        var value = {
            ...
            installation_date: Ext.Date.format(viewModel.get('installationDate'), IMCT.Globals.DateReadFormat),
            removal_date: Ext.Date.format(viewModel.get('removalDate'), IMCT.Globals.DateReadFormat),
    ...
    onAddClick: function () {
        var record = new imct.view.xml.builder.parameter.items.channelequipment.CalibrationDate();
        var store = this.getViewModel().getStore('calibrationDateStore');
